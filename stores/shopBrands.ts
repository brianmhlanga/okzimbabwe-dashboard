...
export const useShopBrandsStore = defineStore('shopBrands', {
    ...
    actions: {
        ...
        async editProductBrand (info:any){
            var data = JSON.stringify({
                "data": info,
            });
            const token = useCookie('token').value || ""
        ...
        async deleteProductBrand (info:any){
            var data = JSON.stringify({
                "data": info,
            });
            const token = useCookie('token').value || ""
        ...
        async getProductsBrandsPagination(page:any) {
            let url = new URL(`${SHOPIFY_URL}/api/product-brands?page=${page}`)
            const token = useCookie('token').value || ""
            var config:any = {
            method: 'GET',
            url: url,
