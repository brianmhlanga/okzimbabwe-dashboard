    ...
    <NuxtLayout name="dashboard">
        <section class="section">
            <div class="container-fluid pt-3">
                <div class="row">
                    <div class="text-900 font-medium text-xl mb-3">Discount Type</div>
                    <div>
                        <div class="card p-4 ">
                        <div class="grid formgrid p-fluid">
                            <div class="field mb-4 col-12 md:col-6">
                                <Button @click="addLineItem = true" label="Add Discount Type" icon="pi pi-plus" class="p-button p-component p-button-secondary p-button-outlined w-auto" secondary />
                            </div>
                             <div class="field mb-4 col-12 md:col-12"> 
                                <DataTable :value="discount_types" ref="dt" class="p-datatable-customers" showGridlines :rows="10"
                                    ...
                                    <template #loading>
                                        Loading discount typess data. Please wait.
                                    </template>
                                    <Column frozen field="name" header="Discount name" style="min-width:12rem">
                                        <template #body="{data}">
                                            {{ data.name }}
                                        </template>
                                    </Column>
                                
                                    <Column frozen field="description" header="Discount method" style="min-width:12rem">
                                        <template #body="{data}">
                                            {{ data.discount_method }}
                                        </template>
                                    </Column>
                                    <Column frozen field="category.name" header="Application Type" style="min-width:12rem">
                                        <template #body="{data}">
        ...
        <Dialog v-model:visible="addLineItem" maximizable modal header="Add New Discount Type" position="top" :style="{ width: '55vw' }">
            <div class="grid formgrid p-fluid">
                <div class="field mb-4 col-12 md:col-6"> 
                ...
                <div class="field mb-4 col-12 md:col-6"> 
                    <label for="company_name" class="font-medium text-900">Discount method</label> 
                    <InputText class="form-control" type="text" v-model="discount_method" />
                </div>
                <div class="field mb-4 col-12 md:col-6"> 
                    <label for="company_name" class="font-medium text-900">Application Type</label> 
                    <InputText class="form-control" type="text" v-model="application_type" />
                </div>
               
                <div class="field mb-4 col-12 md:col-6"> 
                    <label for="company_name" class="font-medium text-900">Is Active</label> 
                    <Dropdown v-model="is_active" :options="options"  checkmark :highlightOnSelect="false"  />
                </div>
            </div>
            <Button :loading="loading" @click="addDiscount()" label="Add Discount Type" icon="pi pi-plus" />
        </Dialog>
...
onMounted(async () => {
    await shopBrandsStore.get_all_discount_types().then((data:any)=>{
            discount_types.value = data.data.data.discounts
        })
...
const addDiscount = async () => {
    ...
    const result = await shopBrandsStore.discount_type(data);
    console.log('result',result.data.success)

    if (result.data.success) {
        toast.add({ severity: 'success', summary: 'Success', detail: 'Discount Type Successfully Added', life: 3000 });
        loading.value = false;
        await shopBrandsStore.getAllCurrencies().then((data:any)=>{
            all_currencies.value = data.data.data.currencies
        })
